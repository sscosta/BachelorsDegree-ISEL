...
extern JASPER_EXPORT void SPCDECL jasperi_new_object_C PROTOTYPE((SP_term_ref jvmref,char *className,char *typesig, SP_term_ref argmap, SP_term_ref args, long numargs, SP_term_ref obj));

/* [PM] 3.8.5 should be called by generated glue code */
extern JASPER_EXPORT unsigned long SPCDECL jasper_glue_push_context(JNIEnv *jnienv);
/* [PM] 3.8.5 should be called by generated glue code */
extern JASPER_EXPORT unsigned long SPCDECL jasper_glue_pop_context(JNIEnv *jnienv, unsigned long context);
/* [PM] 3.8.5 should be called by generated glue code */
extern JASPER_EXPORT int SPCDECL jasper_glue_enter_context_monitor(JNIEnv *jnienv, unsigned long context);
/* [PM] 3.8.5 should be called by generated glue code */
extern JASPER_EXPORT int SPCDECL jasper_glue_leave_context_monitor(JNIEnv *jnienv, unsigned long context);
...
static unsigned long SPJavaGlueLeaveContextMonitor(JNIEnv *jnienv, unsigned long context)
{
  /* no-op */
  return 1;
}
...
static unsigned long SPJavaGlueEnterContextMonitor(JNIEnv *jnienv, unsigned long context)
{
  /* no-op */
...
/* Return 0 on failure (after calling SPJavaGlueError) */
static unsigned long SPJavaGluePushContext(JNIEnv *jnienv)
{
...
/* Return 0 on failure (after calling SPJavaGlueError) */
static unsigned long SPJavaGluePopContext(JNIEnv *jnienv, unsigned long context)
{
...
{
  if ( SP_put_integer(tobj,(obj ? (long)obj : 0))
...
#if !MULTI_SP
/* This cannot be used in Multi-SP since it does not specify SICStus object */
static jobject SPJavaGlueSPCanonicalAtom(JNIEnv *jnienv, unsigned long atom, int argnum)
{
...
/* return 0 on failure (will call SPJavaGlueError) */
static int SPJavaGlueGetAtom(JNIEnv *jnienv, jobject obj /* SPCanonicalAtom or SPTerm */, unsigned long *p_atom, int argnum, const char *err_msg)
{
  int rc = 0;
  ...
  *p_atom = (unsigned long) (*jnienv)->CallLongMethod(jnienv, obj, methodID);
...
static jboolean SPJavaGlueAtomToBoolean(JNIEnv *jnienv, unsigned long atom)
{
...
static int SPJavaGluePostPutChars(JNIEnv *jnienv, jstring string_obj, SP_term_ref tstring, int argnum)
{
  int rc = 0;
  const char *chars = NULL;

  JASPER_DBGLINE(SPJavaGluePostPutChars);
  chars = (*jnienv)->GetStringUTFChars(jnienv, string_obj, NULL);

#if DBG
  fprintf(stderr, "SPJavaGluePostPutChars \"%s\"\n", (chars ? chars : "<<ERROR: chars==NULL!! >>"));
#endif
  JASPER_DBGLINE(SPJavaGluePostPutChars);
  if((!chars)
