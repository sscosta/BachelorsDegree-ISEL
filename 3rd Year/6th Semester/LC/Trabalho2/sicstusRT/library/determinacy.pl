...
tmp_clauses([]).
tmp_clauses([C|Cs]) :-
	assertz(tmp_clause(C)),
	tmp_clauses(Cs).


   %  Avoid infinite error loop when reading past end of file.
recover_error(existence_error(_,_,_,_,past_end_of_stream), _, end_of_file) :- !. % SP
...
detcheck_body_dependencies(once(B1), M, Callchain, Backto0, Backto, SCC0, SCC,
	...
	detcheck_body_dependencies(B1, M, Callchain, Backto0, Backto, SCC0, SCC,
				   ND0, ND).
detcheck_body_dependencies(call(B1), M, Callchain, Backto0, Backto, SCC0, SCC,
		ND0, ND) :-	% SP
    !,
    detcheck_body_dependencies(B1, M, Callchain, Backto0, Backto, SCC0, SCC,
			       ND0, ND).
detcheck_body_dependencies(call_cleanup(B1,_), M, Callchain, Backto0, Backto, SCC0, SCC,
    ...
    detcheck_body_dependencies(B1, M, Callchain, Backto0, Backto, SCC0, SCC,
			       ND0, ND).
detcheck_body_dependencies(call_residue(B1,_), M, Callchain, Backto0, Backto, SCC0, SCC,
		ND0, ND) :-	% SP
    !,
    detcheck_body_dependencies(B1, M, Callchain, Backto0, Backto, SCC0, SCC,
			       ND0, ND).
...
process_dependency(File, Mod, Imports, Where-Level) :-
    absolute_file_name(File, Path, [file_type(source),relative_to(Where)]),
...
cuts_or_fails(-) :- !, fail.  % catch variables
cuts_or_fails(!).
cuts_or_fails(fail).
cuts_or_fails(false).
...
cuts_or_fails(( _ ^ P  )) :- cuts_or_fails(P). % SP
...
cuts_or_fails(call(P)) :- cuts_or_fails(P).
cuts_or_fails(call_cleanup(P,_)) :- cuts_or_fails(P). % SP
cuts_or_fails(call_residue(P,_)) :- cuts_or_fails(P). % SP
...
has_disjunction(( _ ^ P  )) :- has_disjunction(P). % SP
...
has_disjunction(call(P)) :- has_disjunction(P).
has_disjunction(call_cleanup(P,_)) :- has_disjunction(P). % SP
has_disjunction(call_residue(P,_)) :- has_disjunction(P). % SP
...
scan_clauses([], _).
scan_clauses([Cl|Cls], Mod) :-
	clause_body(Cl, Body),	% SP
	scan_body(Body, Mod),
	scan_clauses(Cls, Mod).
...
scan_body(Mod:B, _) :-
    !,
    scan_body(B, Mod).
scan_body(_^B, Mod) :-		% SP
    !,
    scan_body(B, Mod).
scan_body(once(B), Mod) :-
    !,
    scan_body(B, Mod).
scan_body(call(B), Mod) :-	% SP
    !,
    scan_body(B, Mod).
scan_body(call_cleanup(B,_), Mod) :- % SP
    !,
    scan_body(B, Mod).
scan_body(call_residue(B,_), Mod) :- % SP
    !,
    scan_body(B, Mod).
...
calls_known_nondet(-, _, _) :- !, fail.  % catch variables goals.
...
calls_known_nondet(call(Goal), Mod, Pred) :-
    !,
    calls_known_nondet(Goal, Mod, Pred).
calls_known_nondet(call_cleanup(Goal,_), Mod, Pred) :- % SP
    !,
    calls_known_nondet(Goal, Mod, Pred).
calls_known_nondet(call_residue(Goal,_), Mod, Pred) :- % SP
    !,
    calls_known_nondet(Goal, Mod, Pred).
...
hook(user_help, user).
hook(term_expansion(_,_), user).
hook(term_expansion(_,_,_,_), user).
hook(goal_expansion(_,_,_), user).
...
print_decls(All) :-
    (	setof(Pred,
	      ...
	      Ndets),
	(   All == true ->
		format('~n~n% Put these in place of the nondet declarations in~n% ~w~n',
		       [File])
	;   format('~n~n% Insert these nondet declarations in~n% ~w~n',
		   [File])
	),
	member(Pred, Ndets),
...
% :- use_module(library('SU_messages')).   % needed for runtime system only

:- multifile user:message_hook/3.		% turn off Prolog banner in 
:- dynamic   user:message_hook/3.		% stand-alone program version
user:message_hook(_,version(_,_),_) :- !. % SP
