...
prune_domains([], _).
prune_domains([X|Xs], Dom) :- X in Dom, prune_domains(Xs, Dom).
...
my_above(Track0, Track, MaxT) :-
	...
	% plus(Track, D, Track0),	% domain consistent
	D in (inf..NT)\/(1..sup).


% domain consistent addition does not help
plus(X, Y, Z) +:
	Z in dom(X)+dom(Y) !,
	X in dom(Z)-dom(Y) !,
	Y in dom(Z)-dom(X) !.
...
%%%%%%%%%%%%%%%%%%%%%%% enumerate values %%%%%%%%%%%%%%%%%%%%%%% 
route(Lab,Nets) :-
	domain_vars(Nets, Vars),
	labeling(Lab, Vars).

domain_vars([], []).
domain_vars([net(_N,_Head,_Tail,_Terms,_Layer,Track,_Depth)|Nets], [Track|Ts]) :-
	domain_vars(Nets, Ts).
