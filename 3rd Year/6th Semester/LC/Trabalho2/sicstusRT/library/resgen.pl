...
:- public write_elf_resource/2, write_elf_resource/3.
...
write_resource_prefix(SourcePath, SourceName, Output, ResourceKind) :-
   file_property(SourcePath, size(Size)),
   file_property(SourcePath, mod_time(ResourceMTime)),
...
elf_default_machine(HostType, E_MACHINE) :-
   os_type_e_machine(OsType, E_MACHINE1),
   host_os_type(HostType, OsType), !,
...
os_type_e_machine('solaris', 'EM_SPARC').
os_type_e_machine('linux', 'EM_386').
os_type_e_machine('irix', 'EM_MIPS').
os_type_e_machine('win32', 'WIN32'). % only for resource files


%host_type_e_machine('sparc-solaris-5.7', 'EM_SPARC').
%host_type_e_machine('sparc-solaris-5.6', 'EM_SPARC').
%host_type_e_machine('i386-solaris-5.7', 'EM_386').
%host_type_e_machine('i386-solaris-5.6', 'EM_386').
%host_type_e_machine('x86-linux-glibc2.1', 'EM_386').
%host_type_e_machine('x86-linux-glibc2', 'EM_386').
%host_type_e_machine('mips-irix-6.5', 'EM_MIPS').
%host_type_e_machine('x86-win32-nt-4', 'WIN32'). % only for resource files
%host_type_e_machine('x86-win32-nt-5', 'WIN32').
...
elf_const1('SHN_ABS', 16'FFF1, 'Elf32_Half').
elf_const1('SHN_COMMON', 16'FFF2, 'Elf32_Half').
...
elf_const1('EF_MIPS_NOREORDER',     16'00000001, 'Elf32_Word').
elf_const1('EF_MIPS_PIC',           16'00000002, 'Elf32_Word').
elf_const1('EF_MIPS_CPIC',          16'00000004, 'Elf32_Word').
elf_const1('EF_MIPS_XGOT',          16'00000008, 'Elf32_Word').
elf_const1('EF_MIPS_64BIT_WHIRL',   16'00000010, 'Elf32_Word').
elf_const1('EF_MIPS_ABI2',          16'00000020, 'Elf32_Word'). 
elf_const1('EF_MIPS_OPTIONS_FIRST', 16'00000080, 'Elf32_Word'). 

elf_const1('EF_MIPS_ARCH',          16'f0000000, 'Elf32_Word').	% mask: 4 bit field
elf_const1('EF_MIPS_ARCH_1',        16'00000000, 'Elf32_Word').
elf_const1('EF_MIPS_ARCH_2',        16'10000000, 'Elf32_Word').
elf_const1('EF_MIPS_ARCH_3',        16'20000000, 'Elf32_Word').
elf_const1('EF_MIPS_ARCH_4',        16'30000000, 'Elf32_Word').
elf_const1('EF_MIPS_ARCH_5',        16'40000000, 'Elf32_Word').
elf_const1('EF_MIPS_ARCH_6',        16'50000000, 'Elf32_Word').

elf_const1('EF_MIPS_ARCH_ASE',      16'0f000000, 'Elf32_Word').	% mask: 4 bit field
elf_const1('EF_MIPS_ARCH_ASE_MDMX', 16'08000000, 'Elf32_Word').	% multi-media extensions
elf_const1('EF_MIPS_ARCH_ASE_M16',  16'04000000, 'Elf32_Word').	% MIPS16 isa extensions
...
elf_const1('SHT_MIPS_REGINFO', 16'70000006, 'Elf32_Word'). % (SHT_LOPROC + 6)
%% #ifdef __osf__
...
link_elf_desc(Desc, _D,_DT,_Off0,_Off,_C,_CT, _Info) :- var(Desc), !,
   Culprit = link_elf_desc(Desc, _D,_DT,_Off0,_Off,_C,_CT, _Info),
...
eval_elf_expr(Expr, _Value, _Here, _Info) :- var(Expr), !,
   Culprit = eval_elf_expr(Expr, _Value, _Here, _Info),
   barf('Instantiation error: ~w', [Culprit]).
...
eval_elf_expr(st_info(Bind,Type), Value, Here, Info) :-
   ...
   Value is BindV<<4 + (TypeV/\16'F).
eval_elf_expr(r_info(SymbolIndex,Type), Value, Here, Info) :-
   eval_elf_expr(SymbolIndex, SymbolIndexV, Here, Info),
   eval_elf_expr(Type, TypeV, Here, Info),
   Value is SymbolIndexV<<8 + (TypeV/\16'FF).


eval_elf_bool_expr(Expr, _Here, _Info) :- var(Expr), !,
   Culprit = eval_elf_bool_expr(Expr, _Here, _Info),
   barf('Instantiation error: ~w', [Culprit]).
...
win32_resource_file_template(ResourceDataTemplate, _FILE_NAME, RESOURCE_NAME0, FileTemplate, _Info) :-
   ...
   FILEHEADER_Template =
      struct([
              ...
              'Win32_WORD'=16'FFFF, 'Win32_WORD'=16'0000, % TYPE
              'Win32_WORD'=16'FFFF, 'Win32_WORD'=16'0000, % NAME
...
sp_ensure_c_name_code(C, HexCodes,HexCodes1) :-
   memberchk(C, "0123456789abcdefghijklmnopqrstuvxyzABCDEFGHIJKLMNOPQRSTUVXYZ"),
   !,
   HexCodes = [C|HexCodes1].
sp_ensure_c_name_code(C, [0'_, 0'0, 0'x, MSD, LSD|HexCodes],HexCodes) :- % "_0x"
   HiNibble is (C >> 4) /\ 16'F,
   LoNibble is C /\ 16'F,
