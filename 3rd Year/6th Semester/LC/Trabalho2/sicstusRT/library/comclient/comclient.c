...
extern void SPCDECL comclient_create_instance(SP_term_ref tCLSID, SP_term_ref tIID, SP_term_ref tItf, long *result);

// comclient_CLSIDFromProgID(+ProgID:term, -CLSID:term, +atom?:int)
extern void SPCDECL comclient_CLSIDFromProgID(SP_term_ref tProgID, SP_term_ref tCLSID, long atom, long *result);
extern void SPCDECL comclient_ProgIDFromCLSID(SP_term_ref tCLSID, SP_term_ref tProgID, long atom, long *result);

// comclient_INameToIID(+Name:term, -IID:term, +atom?:int)
extern void SPCDECL comclient_INameToIID(SP_term_ref tName, SP_term_ref tIID, long atom, long *result);
// comclient_IIDToName(+IID:term, -Name:term, +atom?:int, -result:int)
extern void SPCDECL comclient_IIDToName(SP_term_ref tIID, SP_term_ref tName, long atom, long *result);

// comclient_invoke(+Obj:term, +Goal:term, +Flags:int, -Result:term, -HResult:integer)
extern void SPCDECL comclient_invoke(SP_term_ref tObj, SP_term_ref tGoal, SP_term_ref tFlags, SP_term_ref tAnswer, long *result);
extern void SPCDECL comclient_invoke_ex(SP_term_ref tObj, SP_term_ref tGoal, SP_term_ref tFlags, SP_term_ref tAnswer, SP_term_ref tResult, long *result);
...
// EXPORTED
long SPCDECL comclient_breakpoints_flag(long on)
{
#if COMCLIENT_DEBUG
...
static unsigned long atom_comclient_error;
static unsigned long atom_comclient_object;
...
static int SysAllocStringSP(char *s, BSTR *pBSTR)
{
  BSTR bstr = NULL;
  int len1, len1x;
  UINT CodePage;
...
static char *AllocTmpStringSize(long size)
{
...
static int SPPutBSTR(SP_term_ref t, BSTR bstr, long atom)
{
  int rc = 1;
  ...
  else                          // make list
    {
      ...
      if (! (len > 0) )
      ...
      else
        {
          ...
          for (i = len; i;)
            {
              if (!SP_put_integer(tmp, (long) bstr[--i]))
                {
                  rc = 0;
                  break;
                }
...
void SPCDECL comclient_CLSIDFromProgID(SP_term_ref tProgID, SP_term_ref tCLSID, long atom, long *result)
{
  HRESULT hr;
  BSTR progID = NULL;
  BSTR clsidBSTR = NULL;
...
// list or atom depending on the atom arg).
void SPCDECL comclient_ProgIDFromCLSID(SP_term_ref tCLSID, SP_term_ref tProgID, long atom, long *result)
{
  HRESULT hr;
  BSTR progID = NULL;
  BSTR clsidBSTR = NULL;
...
static HRESULT INameToIID (char *iname, IID *pIID)
{
  ...
  rc = RegOpenKeyEx(
                    ...
                    "SOFTWARE\\Classes\\Interface", // LPCTSTR lpSubKey,  // subkey name
...
void SPCDECL comclient_INameToIID(SP_term_ref tName, SP_term_ref tIID, long atom, long *result)
{
  LPOLESTR olestr = NULL;
  BSTR bstrIID = NULL;
  char *name;
...
void SPCDECL comclient_IIDToName(SP_term_ref tIID, SP_term_ref tName, long atom, long *result)
{
  BSTR bstrIID = NULL;
  char *szIID;
  IID iid;
  ...
  if (( atom ? SP_put_string(tName, szIID) : SP_put_list_chars(tName, tail, szIID)))
    {
      hr = DISP_E_TYPEMISMATCH; // Hmm
      goto cleanup;
    }
                             ...
                             REGSAM samDesired) // at least KEY_READ
{
  ...
  hr = RegOpenKeyEx(
                    ...
                    subKey, // LPCTSTR lpSubKey,  // subkey name
                    0, // DWORD ulOptions,   // reserved
  ...
  if (old_way)
  ...
  else
    {
      ...
      hr = CoCreateInstanceEx(
                              ...
                              // be instantiated
                              1, // ULONG cmq,                   //Number of MULTI_QI structures in 
                              // pResults
                              results // MULTI_QI *pResults           //Array of MULTI_QI structures
                              );
      if (SUCCEEDED(hr))
        {
          *ppItf = results[1].pItf;
        }
    }
...
static SP_external_object_link *register_idispatch(IDispatch *pDisp)
{
 ...
 cleanup:
  if (pUnkn) (void) IUnknown_Release(pUnkn);
  return pObj;
}
     
#if 0
static IUnknown *lookup_iunknown(long obj)
{
  SP_external_object_link *pObj;
...
static IUnknown *comclient_get_interface(SP_term_ref tObj)
{
  ...
  if (pUnkn) IUnknown_AddRef(pUnkn); /* 21 Aug 2000 */
...
void SPCDECL comclient_create_instance(SP_term_ref tCLSID, SP_term_ref tIID, SP_term_ref tItf, long *result)
{
  CLSID clsid;
  IID iid;
  IUnknown *pItf = NULL;
...
void SPCDECL comclient_get_active_object(SP_term_ref tCLSID, SP_term_ref tIID, SP_term_ref tItf, long *result)
{
  CLSID clsid;
  IID iid;
         ...
         int dispatch)
{
 ...
 cleanup:
  ...
  if (pDsp)
    {
      IDispatch_Release(pDsp);
    }
  if (pTInfo)
    {
      if (pTypeAttr)
        {
          (void) ITypeInfo_ReleaseTypeAttr(pTInfo, pTypeAttr);
        }
    }
...
static HRESULT SPPutVariant(SP_term_ref t, VARIANT *pVar)
{
  ...
  switch (V_VT(pVar) & VT_TYPEMASK) // mask away VT_BYREF
    {
    case VT_UI8:
    case VT_I8:
    case VT_I4:
    ...
    case VT_BOOL:
      {
        ...
        if (!SP_put_integer(t, (long)(b != VARIANT_FALSE)))
          {
            hr = DISP_E_OVERFLOW; // Hmm
            goto cleanup;
          }
    ...
    case VT_UNKNOWN:
      {
        IUnknown *pUnkn = (V_ISBYREF(pVar) ? *V_UNKNOWNREF(pVar) : V_UNKNOWN(pVar));
        SP_external_object_link *pObj;
    ...
    case VT_DISPATCH:
      {
        IDispatch *pDisp = (V_ISBYREF(pVar) ? *V_DISPATCHREF(pVar) : V_DISPATCH(pVar));
        SP_external_object_link *pObj;
      ...
      break;
    case VT_ARRAY:              // for now
      hr = DISP_E_TYPEMISMATCH;
      goto cleanup;
      break;
    case 0: // *    PVOID          VT_BYREF (Generic ByRef)
      hr = DISP_E_TYPEMISMATCH; // for now
      goto cleanup;
      break;
    }
...
static HRESULT SPInvoke(SP_term_ref tObj, SP_term_ref tGoal, SP_term_ref tFlags, SP_term_ref tAnswer)
{
  ...
  // atom no longer used, no need to register it
  ...
  */
  ...
  if (pDsp) IDispatch_Release(pDsp);
  if (pUnkn) IUnknown_Release(pUnkn);
      ...
      if (hr == DISP_E_EXCEPTION)
        {
          ...
          if (!(
                ...
                && SP_put_integer(tTmp, (long) pExcepInfo->wCode)
                && SP_unify(tArg, tTmp)
                && SP_get_arg(argNo++, tResult, tArg)
                ...
                && SP_unify(tArg, tTmp)
                && SP_get_arg(argNo++, tResult, tArg)
                && SP_put_integer(tTmp, (long) pExcepInfo->dwHelpContext)
                && SP_unify(tArg, tTmp)
                && SP_get_arg(argNo++, tResult, tArg)
                && SP_put_integer(tTmp, (long) pExcepInfo->scode)
                && SP_unify(tArg, tTmp)
                ))
            {
              goto cleanup;
          ...
          if ((hr == DISP_E_PARAMNOTFOUND || hr == DISP_E_TYPEMISMATCH)
              && argErr <= (unsigned int) arity )
            {
              ...
              if (!(
                    ...
                    && SP_put_integer(tTmp, (long) argErr)
                    && SP_unify(tArg, tTmp)
                    ))
                {
                  goto cleanup;
...
void SPCDECL comclient_invoke(SP_term_ref tObj, SP_term_ref tGoal, SP_term_ref tFlags, SP_term_ref tAnswer, long *result)
{
  HRESULT hr;
...
void SPCDECL comclient_invoke_ex(SP_term_ref tObj, SP_term_ref tGoal, SP_term_ref tFlags, SP_term_ref tAnswer, SP_term_ref tResult, long *result)
{
  HRESULT hr;
...
void SPCDECL comclient_equal(SP_term_ref tObj1, SP_term_ref tObj2, long *pResult)
{
 ...
 cleanup:
  if (pUnkn1a) IUnknown_Release(pUnkn1a);
  if (pUnkn1b) IUnknown_Release(pUnkn1b);
  if (pUnkn2a) IUnknown_Release(pUnkn2a);
  if (pUnkn2b) IUnknown_Release(pUnkn2b);
  *pResult = result;
}


void SPCDECL comclient_valid(SP_term_ref tObj, long *pResult)
{
  IUnknown *pUnkn = NULL;
  int result = 0;
...
long SPCDECL comclient_garbage_collect(void)
{
...
void SPCDECL finalize_external_object(SP_term_ref tObj, long *pexisted)
{
