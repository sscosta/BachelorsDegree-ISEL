...
VOID Schedule () {
	...
	RunningThread->Running += (GetTickCount() - RunningThread->LastChanged);
	RunningThread->LastChanged = GetTickCount();

	NextThread->State = Running;
	NextThread->Ready += (GetTickCount() - NextThread->LastChanged);
	NextThread->LastChanged = GetTickCount();
...
VOID UtExit () {
	...
	self->Running += (GetTickCount() - self->LastChanged);
	self->LastChanged = GetTickCount();
	while (!IsListEmpty(&self->JoinList)) {
		// Remover da cabeça de JoinList
		PUTHREAD newRdy = CONTAINING_RECORD(RemoveHeadList(&self->JoinList), UTHREAD, Link);
		// Adicionar à cauda de ReadyQueue
		UtActivate(newRdy);
	}
	printf("Uthread Elapsed Time (Actual) = %lu\n", UtGetTimeAlive(self) + UtGetTimeAlive(self) + UtGetTimeAlive(self));
	printf("Thread %c Ready: %lu\n", self->Argument, UtGetTimeReady(self));
	printf("Thread %c Blocked: %lu\n", self->Argument, UtGetTimeBlocked(self));
	printf("Thread %c Active: %lu\n", self->Argument, UtGetTimeAlive(self));
	RemoveEntryList(&self->AliveList);
...
VOID UtActivate (HANDLE ThreadHandle) {
	PUTHREAD pthread = ((PUTHREAD)ThreadHandle);
	pthread->LastChanged = GetTickCount();
	pthread->State = Ready;
...
HANDLE UtCreate (UT_FUNCTION Function, UT_ARGUMENT Argument) {
	...
	InsertHeadList(&AliveQueue, &Thread->AliveList);
...
int UtJoin(HANDLE thread) {
	...
	if (!UtAlive(thread))
		return 1;
	// Bloquear thread
	InsertTailList(&pthread->JoinList, &self->Link);

	self->State = Blocked;
	self->Running += (GetTickCount() - self->LastChanged);
	self->LastChanged = GetTickCount();

	self->JoinCanceled = FALSE;
	UtDeactivate();

	self->Blocked += (GetTickCount() - self->LastChanged);
	self->LastChanged = GetTickCount();

	return self->JoinCanceled ? -1 : 1;
}
BOOL UtJoinCancel(HANDLE thread) {
	if (!UtAlive(thread) || UtGetState(thread) != Blocked)
		return FALSE;
	PUTHREAD pthread = (PUTHREAD)thread;
	RemoveEntryList(&pthread->Link);
	pthread->JoinCanceled = TRUE;
	pthread->Blocked += (GetTickCount() - pthread->LastChanged);
	UtActivate(thread);
	return TRUE;
}

//1. a) add function to api
INT UtGetState(HANDLE thread) {
	return ((PUTHREAD)thread)->State;
}
//1. b) Uma thread está viva se:
//		- o seu estado é running e é a thread que chama esta função
//		- o seu estado é ready e está na ready queue
//		- o seu estado é blocked e está numa join_list
//bool utalive2(handle thread) {
//	puthread pthread = (puthread)thread;
//	if (utself() == thread && pthread->state == running)
//		return true;
//	if (pthread->state == ready && isinlist(&readyqueue, thread))
//		return true;
//	puthread self = (puthread)utself();
//	if (pthread->state == blocked && (isinlist(&self->joinlist, thread) || isinblockedlist(&readyqueue, thread)))
//		return true;
//	return false;
//}
BOOL UtAlive(HANDLE thread) {
	...
	while (curr != &AliveQueue) {
		if (CONTAINING_RECORD(curr, UTHREAD, AliveList) == thread)
			return TRUE;
		curr = curr->Flink;
	}
	return FALSE;
...
BOOL IsInList(PLIST_ENTRY head, HANDLE thread) {
	PLIST_ENTRY curr = head->Flink;
	while (curr != head) {
		if (CONTAINING_RECORD(curr, UTHREAD, Link) == thread)
			return TRUE;
		curr = curr->Flink;
	}
	return FALSE;
...
BOOL IsInBlockedList(PLIST_ENTRY head, HANDLE thread) {
	PLIST_ENTRY currNode = head->Flink;
	while (currNode != head) {
		PUTHREAD uthread = (PUTHREAD)CONTAINING_RECORD(currNode, UTHREAD, Link);
		if (!IsListEmpty(&uthread->JoinList) && IsInList(&uthread->JoinList, thread))
			return TRUE;
...
CHAR * UtStateToString(INT threadState) {
	THREAD_STATE ts = (THREAD_STATE)threadState;
...
ULONG UtGetTimeReady(HANDLE h) {
	return ((PUTHREAD)h)->Ready;
}
ULONG UtGetTimeBlocked(HANDLE h) {
	return ((PUTHREAD)h)->Blocked;
}
ULONG UtGetTimeAlive(HANDLE h) {
	PUTHREAD pthread = (PUTHREAD)h;
	ULONG TimeAlive = pthread->Blocked + pthread->Ready + pthread->Running;
	return TimeAlive;
}
//1. d)

VOID UtTerminate(HANDLE thread) {
	NumberOfThreads -= 1;

	PUTHREAD pthread = (PUTHREAD)thread;
	pthread->Running += (GetTickCount() - pthread->LastChanged);
	pthread->LastChanged = GetTickCount();
	while (!IsListEmpty(&pthread->JoinList)) {
		// Remover da cabeça de JoinList
		PUTHREAD newRdy = CONTAINING_RECORD(RemoveHeadList(&pthread->JoinList), UTHREAD, Link);
		// Adicionar à cauda de ReadyQueue
		UtActivate(newRdy);
	}
	RemoveEntryList(&pthread->AliveList);
}

VOID UtFinish() {
	while (!IsListEmpty(&AliveQueue)) {
		PUTHREAD pthread = CONTAINING_RECORD(RemoveHeadList(&AliveQueue), UTHREAD, AliveList);
		if (UtSelf() != pthread) {
			RemoveEntryList(&pthread->Link);
			UtTerminate(pthread);
